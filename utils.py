...
def calculate_area(polygon):
    """计算多边形的面积（可以用向量叉积法来计算）"""
    area = 0.0
    for i in range(len(polygon)):
        p1 = polygon[i].start_point
        p2 = polygon[i].end_point
        area += (p1.x * p2.y - p2.x * p1.y)
    return abs(area) / 2.0

def calculate_sector_area(ref):
    """计算圆弧和圆心围成的扇形面积"""
    radius = ref.radius
    start_angle = ref.start_angle
    end_angle = ref.end_angle
    ...
    # 计算扇形面积
    area = 0.5 * radius ** 2 * theta_rad
    return area

def remove_unfit_polygons(polys, tolerance1=1000, tolerance2 = 1000):
    """
    去除相似多边形，基于几何特征（周长、面积、顶点数量等）
    :param polys: 多边形列表
    :param tolerance: 容差
    :return: 保留的多边形列表
    """
    ...
    for poly in polys:
        ...
        arc_area = 0.0
        for seg in poly:
            if seg.ref not in refs:
                refs.append(seg.ref)
        for ref in refs:
            if isinstance(ref, DArc):
                arc_area += calculate_sector_area(ref)
        if(arc_area / area > 0.005):
            fit_polys.append(poly)
    return fit_polys
