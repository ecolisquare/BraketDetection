...
class SweepIntersector():
    ...
    def findIntersections(self,origSegList,lines_dict,points_dict):
        """
        ...
        while not self.X_structure.empty():
            ...
            if sitPred is not None:
                ...
                self.computeIntersection(sitPred,lines_dict,points_dict)
                sit = self.Y_structure.pred(sitSucc)
                if sit != sitPred:
                    self.computeIntersection(sit,lines_dict,points_dict)
    ...
    def computeIntersection(self,sit0,lines_dict,points_dict):
        ...
        if Segment.orientation(s0,s1.end()) <= 0 and Segment.orientation(s1,s0.end()) >= 0:
            ...
            if it is not None:
                ...
                if q:
                    ...
                    # lines dictionary
                    # if self.original[s0] not in lines_dict:
                    #     lines_dict[self.original[s0]] = []
                    # if self.original[s1] not in lines_dict:
                    #     lines_dict[self.original[s1]] = []
                    # lines_dict[self.original[s1]].append(self.original[s0])
                    # lines_dict[self.original[s0]].append(self.original[s1])
                    #points dictionary
                    # insert intersection point into result dictionary
